...
async function run() {
  ...
  const continuityRehearsal = page.getByTestId("admin-live-continuity-rehearsal");
  await continuityRehearsal.waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("Live Continuity Rehearsal Packet", { exact: true }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("Learner state survives only through the authenticated account", { exact: true }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("AI teacher cannot run without the live limiter", { exact: true }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("Live AI teacher is provider-backed but still safe", { exact: true }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("Google login returns to the deployed portal", { exact: true }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("Different learners cannot see each other's state", { exact: true }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("Public launch stays locked after controlled testing", { exact: true }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("SUPABASE_SECRET_KEY", { exact: false }).waitFor({ timeout: 15000 });
  await continuityRehearsal.getByText("GEMINI_API_KEY", { exact: false }).waitFor({ timeout: 15000 });
