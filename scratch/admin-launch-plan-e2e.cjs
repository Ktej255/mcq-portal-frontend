...
async function run() {
  ...
  await page.getByText("Geography local pilot", { exact: true }).waitFor({ timeout: 15000 });
  ...
  await verdictPanel.getByText("24 May to 25 May operating answer", { exact: true }).waitFor({ timeout: 15000 });
  ...
  await verdictPanel.getByText("Geography first", { exact: true }).waitFor({ timeout: 15000 });
  ...
  await actionQueue.getByText("Finish founder checklist", { exact: true }).waitFor({ timeout: 15000 });
  await actionQueue.getByText("Invite the first tiny tester group", { exact: true }).waitFor({ timeout: 15000 });
  await actionQueue.getByText("Load fresh advanced GEO-D01 MCQs", { exact: true }).waitFor({ timeout: 15000 });
  ...
  await sharePacket.getByText("Start Watch room", { exact: true }).waitFor({ timeout: 15000 });
