...
const evidencePath = path.join(__dirname, "history-loop-e2e-evidence.json");
...
async function selectCommandDay(page, day, title) {
  ...
  await page.getByText(`Day ${day} of 60`, { exact: false }).waitFor({ timeout: 15000 });
}
...
async function run() {
  ...
  await page.goto(`${baseUrl}/upsc/history`, { waitUntil: "networkidle" });
  await page.evaluate((key) => window.localStorage.removeItem(key), storageKey);
  await page.reload({ waitUntil: "networkidle" });
  await page.getByRole("link", { name: /Watch\s+Class/i }).first().waitFor({ timeout: 15000 });
  await selectCommandDay(page, 4, "Revolt of 1857");
  await assertNoOverflow(page, "history-command-day-4", checks);

  await page.getByRole("link", { name: /Lab\s+Visual/i }).first().click();
  await page.waitForURL("**/upsc/history/lab?mode=modern-timeline&day=4", { timeout: 15000 });
  await page.getByText("Timeline To Cause", { exact: false }).first().waitFor({ timeout: 15000 });
  await assertNoOverflow(page, "history-lab-day-4", checks);

  await page.getByRole("link", { name: /MCQ\s+Practice/i }).first().click();
  await page.waitForURL("**/upsc/history/mcq-readiness?day=4", { timeout: 15000 });
  await page.getByText("Plan the fresh MCQ batch", { exact: false }).first().waitFor({ timeout: 15000 });
  await assertNoOverflow(page, "history-mcq-readiness-day-4", checks);

  await page.getByRole("link", { name: /History command room/i }).first().click();
  await page.waitForURL("**/upsc/history", { timeout: 15000 });
  await selectCommandDay(page, 4, "Revolt of 1857");
  await page.getByRole("link", { name: /Watch\s+Class/i }).first().click();
  await page.waitForURL("**/upsc/history/watch?day=4", { timeout: 15000 });
  await page.getByText("Build the topic before testing", { exact: false }).first().waitFor({ timeout: 15000 });
  await assertNoOverflow(page, "history-watch-day-4", checks);
  ...
  await page.waitForURL("**/upsc/history/talk?day=4", { timeout: 15000 });
  await page.getByText("Socratic Talk", { exact: false }).first().waitFor({ timeout: 15000 });
  await assertNoOverflow(page, "history-talk-day-4", checks);
  await page.getByPlaceholder("Write the explanation in your own words.").fill(
    "I only remember a vague event and need to study the class again."
  );
  ...
  if (
    ...
    throw new Error(`History Talk progress did not persist correctly: ${JSON.stringify(queuedProgress)}`);
  }

  await page.getByRole("link", { name: /Track\s+Progress/i }).first().click();
  await page.waitForURL("**/upsc/history/track", { timeout: 15000 });
  await page.getByText("Track History command", { exact: false }).first().waitFor({ timeout: 15000 });
  await page.getByText("Day 4", { exact: false }).first().waitFor({ timeout: 15000 });
  await assertNoOverflow(page, "history-track-with-revisit-day-4", checks);

  await page.getByRole("link", { name: /Revisit\s+Repair/i }).first().click();
  await page.waitForURL("**/upsc/history/revisit?day=4", { timeout: 15000 });
  await page.getByText("Focused recovery", { exact: false }).first().waitFor({ timeout: 15000 });
  await assertNoOverflow(page, "history-revisit-day-4", checks);
  await page.getByPlaceholder("Write the recovery note or corrected explanation here.").fill(
    "Recovered: Revolt of 1857 answer should connect causes, centres, leaders, nature, failure, consequences, and chronology."
  );
  ...
  if (recoveredProgress?.revisitQueued !== false || recoveredProgress?.confidence !== "Working") {
    throw new Error(`History revisit recovery did not persist correctly: ${JSON.stringify(recoveredProgress)}`);
  }

  await page.goto(`${baseUrl}/upsc/history/track`, { waitUntil: "networkidle" });
  await page.getByText("Track History command", { exact: false }).first().waitFor({ timeout: 15000 });
  await assertNoOverflow(page, "history-track-after-recovery", checks);
  ...
  await page.screenshot({ path: path.join(__dirname, "history-loop-wiring-final.png"), fullPage: true });
  await browser.close();
