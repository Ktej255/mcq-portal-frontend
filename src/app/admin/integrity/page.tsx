...
const integrityEvents = [
  ...
  {
    label: "Backend sync",
    detail: "Legacy MCQ publishing API is disabled by default until an approved authenticated replacement is verified.",
    status: "ISOLATED",
  },
];
...
export default function AdminIntegrityPage() {
  return (
    <div data-testid="admin-integrity-page" className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <Badge variant="outline" className="flex h-11 w-fit items-center gap-2 rounded-md border-emerald-300 bg-emerald-50 px-4 font-bold text-emerald-900 dark:border-emerald-900/60 dark:bg-emerald-950/20 dark:text-emerald-100">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-lg border border-emerald-100 bg-emerald-50 p-6 text-emerald-950 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-50">
        ...
        <div className="rounded-lg border border-amber-100 bg-amber-50 p-6 text-amber-950 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-50">
        ...
        <div className="rounded-lg border border-blue-100 bg-blue-50 p-6 text-blue-950 dark:border-blue-900/50 dark:bg-blue-950/20 dark:text-blue-50">
        ...
        <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
          {integrityEvents.map((event) => (
            <div key={event.label} className="flex flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between">
              ...
              <Badge variant={event.status === "ISOLATED" ? "outline" : "secondary"} className="w-fit font-bold">
                {event.status}
              </Badge>
            </div>
          ))}
